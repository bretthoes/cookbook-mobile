...
export {
  draftItemHasContent,
  draftItemHasContent as draftHasContent,
  getCurrentWeekKey,
  hasDraftContent,
  WEEKLY_IMPORT_LIMIT,
  type DraftFormData,
  ...
  persist(
    (set, get) => ({
      ...
      saveDraft: (cookbookId, formData) => {
        ...
        const draftFields = buildDraftFieldsFromFormData(formData)

        if (!draftItemHasContent(draftFields)) {
          get().deleteDraft(cookbookId)
          return
        }
        ...
        const persistedFields = {
          ...draftFields,
          savedAt: savedAt.toISOString() as unknown as Date,
        }

        if (existing) {
          set({
            drafts: get().drafts.map((d) =>
              d.cookbookId === cookbookId ? { ...d, ...persistedFields } : d,
            ),
          })
        } else {
          set({
      ...
      pruneEmptyDrafts: () => {
        const contentful = get().drafts.filter((d) => draftItemHasContent(d))
        if (contentful.length !== get().drafts.length) {
          set({ drafts: contentful })
        }
      },
      ...
      migrateFromLegacySnapshot: (snapshot) => {
        ...
        if (rs) {
          if (Array.isArray(rs.drafts) && rs.drafts.length > 0) {
            patch.drafts = (rs.drafts as RecipeDraftItem[]).filter((d) => draftItemHasContent(d))
          }
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          const contentful = state.drafts.filter((d) => draftItemHasContent(d))
          if (contentful.length !== state.drafts.length) {
            state.drafts = contentful
          }
        }
      },
    },
