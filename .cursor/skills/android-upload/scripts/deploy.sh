...
usage() {
  cat <<'EOF'
Usage: bash .cursor/skills/android-upload/scripts/deploy.sh [options]

Options:
  --pull              git pull --ff-only before building
  --build-only        Build on EAS; do not submit to Google Play
...
Examples:
  ...
  bash .cursor/skills/android-upload/scripts/deploy.sh --submit-id 903f0ba3-55ec-4445-bda0-2d376d4263e4
EOF
}
...
STATUS="FAILED"
STEP="init"
PACKAGE_NAME="com.cookbookmobile"
PLAY_CONSOLE_URL="https://play.google.com/console"

report() {
  cat <<EOF

=== Android Deploy Report ===
Status:       ${STATUS}
Step:         ${STEP}
Commit:       ${COMMIT}
...
Play Console: ${PLAY_CONSOLE_URL}
Next:         Google Play processing can take minutes to hours; check the configured track (internal by default) before promoting.
EOF
}
...
if [ -z "$SUBMIT_BUILD_ID" ]; then
  STEP="eas-build"
  BUILD_LOG="$(mktemp)"
  if ! eas build --platform android --profile production --non-interactive 2>&1 | tee "$BUILD_LOG"; then
    rm -f "$BUILD_LOG"
    exit 1
  fi
...
STEP="eas-submit"
SUBMIT_LOG="$(mktemp)"
if ! eas submit --platform android --profile production --id "$BUILD_ID" --non-interactive 2>&1 | tee "$SUBMIT_LOG"; then
  rm -f "$SUBMIT_LOG"
  exit 1
fi
...
if [ "$VERSION_CODE" = "?" ]; then
  VERSION_CODE="$(grep -Ei 'version code|Version code' "$SUBMIT_LOG" | tail -1 | sed -E 's/.*[^0-9]([0-9]+)[^0-9]*$/\1/' || echo "?")"
fi

if [ "$VERSION" = "?" ]; then
  VERSION="$(grep -Ei 'app version|App version' "$SUBMIT_LOG" | tail -1 | sed -E 's/.*:[[:space:]]*//' || echo "?")"
fi
