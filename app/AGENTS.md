...
| ----------------------------------------------------------------------------------------------- | ---------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| `_layout.tsx`                                                                                   | —                                                    | Root: fonts, i18n, app init (`useAppInit`), `QueryProvider`, `ThemeProvider`, `<Slot />` |
...
- **Authenticated screens** live under `app/(logged-in)/`. The layout at `(logged-in)/_layout.tsx` enforces auth via `useIsAuthenticated()` from `stores/authStore`.
...
5. Read client state from `stores/*` (Zustand) and server state from `hooks/queries/*` (TanStack Query).
...
2. Default-export a function component; use Zustand selectors or query hooks as needed.
...
| Topic                | Location                                  |
| -------------------- | ----------------------------------------- |
| Zustand stores       | `stores/AGENTS.md`                        |
